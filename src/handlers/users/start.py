...
async def start_deep_link(message: Message, command: CommandObject):
    ...
    if check_user == False:
        ...
        await DataBase.add_user(user_tg_id=user_tg_id, username=username, register_date=current_date)
    if command.args != None:
        permission_code = command.args
        if await DataBase.check_permission_code(permission_code=permission_code) == True:
            await DataBase.give_mark_permission(user_tg_id=message.from_user.id)
