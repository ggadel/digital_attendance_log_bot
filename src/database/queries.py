...
class DataBase:
    ...
    @staticmethod
    async def add_user(user_tg_id, username, register_date):
        async with async_engine.connect() as conn:
            stmt = insert(users).values(
                [
                    {'user_tg_id': user_tg_id, "username": username, "register_date": register_date},
                ]
            )
    ...
    @staticmethod
    async def give_mark_permission(user_tg_id):
        async with async_engine.connect() as conn:
            stmt = (
                update(users)
